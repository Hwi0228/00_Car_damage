...
#show: resume.with(font: "Pretendard")

#set text(lang: "ko")
#outline()
typst로 만들었습니다.
= 문제 정의서
아빠는 #strong[교통사고가 발생해 차량에 손상을 입어도 어떻게
수리해야 하는지, 누구에게 책임을 져야 하는지를 모르기 때문];에 문제점을
겪고 있으며, 이를 #strong[세그멘테이션을 사용한 수리 방법 제안과 객체
추적을 사용한 용의자 추적] 으로 해결하고자 한다.

== 객체 추적
문콕 및 스크래치가 발생했을 때, 범인이 누군지를 차량의 블랙박스 영상을
넣으면 추적한다.

== 세그멘테이션
차량의 사진을 찍으면 세그멘테이션을 통해 어떤 부위가 어떻게 손상되었는지
알려주고, 어떻게 수리하면 좋을지도 알려준다.

== 워크플로우
#box(image("image_01_workflow_diagram.png"))
== 폴더 구조
#let folder-icon = "📁"
#dtree(icons: (
    "dir": folder-icon,
...
== 개발 환경
깃허브: https://github.com/Hwi0228/00_Car_damage
...
= 웹 서버
- flask를 사용할 예정 - gradia는 너무 확장성이 적음, jinja 템플릿은
  LLM에게(tailwind css)
- 서버는 #strike[PythonAnywhere? Render?] ⇒ #strike[Hugging Face with
  Flask] ⇒ CPU Instance가 너무 적음, Render
...
- client-side ai로
  ...
  2026-09-12T01:22:52.565278151Z Press CTRL+C to quit
  ...
  2026-09-12T01:23:12.419978892Z 127.0.0.1 - - [12/Sep/2026 01:23:12] "GET /favicon.ico HTTP/1.1" 404 -
  ```

== UI 구상
  #box(image("image_02_ui_document.png"))
= 세그멘테이션
...
= 테스트 계획
- 나 말고 다른 사람이 아파트 주차장에서 스크래치 있는 차
  확인해서 사진 찍어보기
